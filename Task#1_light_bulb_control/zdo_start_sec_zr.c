...
void send_in_cycle(zb_uint8_t param) ZB_CALLBACK {
    ZB_GET_OUT_BUF_DELAYED(send_data);
    ZB_SCHEDULE_ALARM(send_in_cycle,0,time_interval*ZB_TIME_ONE_SECOND);
    time_interval++;
}
...
void zb_zdo_startup_complete(zb_uint8_t param) ZB_CALLBACK
{
  ...
  if (buf->u.hdr.status == 0)
  {
   ...
   // send_data((zb_buf_t *)ZB_BUF_FROM_REF(param));
    ZB_SCHEDULE_CALLBACK(send_in_cycle,0);
  }
  else
  {
...
void data_indication(zb_uint8_t param)
{
  ...
  // send_data(asdu);
  //ZB_SCHEDULE_ALARM(send_data,param,N++*ZB_TIME_ONE_SECOND);
}
#endif
